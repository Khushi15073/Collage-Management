...
export const DEFAULT_PERMISSIONS: PermissionDefinition[] = DEFAULT_PERMISSION_GROUPS.flatMap(
  (group) => group.permissions.map((permission) => ({ ...permission, section: group.section }))
);

export function getPermissionDefinition(permissionName: string) {
  return DEFAULT_PERMISSIONS.find((permission) => permission.key === permissionName) || null;
}

export function formatPermissionLabel(permissionName: string) {
